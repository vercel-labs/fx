...
The supported protocol is `http/json`; it is the default when the protocol variables are unset. Signal-specific variables override their generic counterparts. The prototype supports:
...
- `OTEL_SERVICE_NAME` and `OTEL_SDK_DISABLED`

Compression and custom certificates are not supported yet. Spans are best effort: they are queued after turn completion, retried only within one bounded timeout, and flushed with a bounded wait at shutdown. They cover interactive turns, `fx ask`, and ACP requests.
