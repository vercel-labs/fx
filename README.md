...
The `subagent` tool has two operations: `run` delegates one temporary task, and `message` creates or continues a named persistent agent. Each call waits for the child's result. A first message creates the named child immediately; optional instructions set or replace that child's system overlay while preserving fx's trusted base prompt. Failed calls include the captured failure reason and any partial result; earlier tool effects are not rolled back or automatically retried. Child sessions remain private to their saved parent session. Existing child records remain readable, but records saved by this version cannot be reopened by older binaries that only support child registry schema 1.
