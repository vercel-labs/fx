...
const read_file_description =
    "Read one UTF-8 text file with bounded line-numbered output and optional start_line/line_count range, or display one known PNG file inline in a compatible terminal. Paths may be workspace-relative or external using an absolute path, ~/..., or a relative workspace escape such as ../...; external access is subject to permission policy. When to use: inspect an exact known path before editing or explaining code, or show a requested PNG inline. When NOT to use: list directories, search many files, read other binary formats, or bypass dedicated search tools.";
