...
pub const slash_specs = [_]SlashSpec{
    ...
    .{ .kind = .btw, .command = "/btw", .help_entry = "/btw <question>", .completion_description = "ask a non-interrupting side question without tools or history", .presentation_category = .general, .has_args = true, .accepts_payload = true },
    .{ .kind = .recap, .command = "/recap", .help_entry = "/recap", .completion_description = "readonly recap without history or compact", .presentation_category = .session, .has_args = true, .accepts_payload = true },
