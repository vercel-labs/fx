...
pub const top_level_specs = [_]TopLevelSpec{
    ...
    .{
        .kind = .worktree,
        .token = "worktree",
        .usage = "worktree [list|status] [--json] | worktree create <branch> <path> [--base <ref>] [--start] [--json] | worktree open <branch|path> [--resume] | worktree remove <branch|path> --confirm [--force] [--json]",
        .summary = "Manage Git worktrees and launch fx inside them",
        .options = &.{
            .{ .flag = "list|status", .description = "Show every worktree and identify the active one (default)" },
            .{ .flag = "create <branch> <path>", .description = "Create a linked worktree for a new or existing branch" },
            .{ .flag = "--base <ref>", .description = "Base a new branch on a specific revision" },
            .{ .flag = "--start", .description = "Start a new interactive fx session in the created worktree" },
            .{ .flag = "open <branch|path>", .description = "Start fx in an existing worktree" },
            .{ .flag = "--resume", .description = "Resume the latest session associated with the selected worktree" },
            .{ .flag = "remove <branch|path> --confirm", .description = "Remove a linked worktree without deleting its branch" },
            .{ .flag = "--force", .description = "Allow Git to remove a dirty or locked linked worktree" },
            json_option,
        },
        .details = &.{
            "Removal never deletes the branch and refuses the primary or currently active worktree.",
            "Interactive launches inherit the selected worktree as their workspace root, so saved sessions resume there.",
        },
    },
    .{
        .kind = .workspace,
