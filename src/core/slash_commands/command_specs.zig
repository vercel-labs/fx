...
test "rendered top-level help is a complete CLI navigation page" {
    ...
    try std.testing.expect(std.mem.find(u8, text, "di starts an interactive session by default.") != null);
    ...
    try std.testing.expect(std.mem.find(u8, text, "Source and issues:  https://github.com/lee101/di") != null);
    try std.testing.expect(std.mem.find(u8, text, "\nReport a problem:   run `/feedback` inside di\n") != null);
    try std.testing.expect(std.mem.find(u8, text, "\n\n\nRun `di <command> --help`") == null);
...
test "terminal top-level help adds styling without changing visible content" {
    ...
    try std.testing.expect(std.mem.find(u8, terminal, "\x1b[38;5;243mUniversal, native coding agent") != null);
    try std.testing.expect(std.mem.find(u8, terminal, "\x1b[4mhttps://github.com/lee101/di\x1b[0m") != null);
    try std.testing.expect(std.mem.find(u8, terminal, "\x1b[39mrun `/feedback` inside di\x1b[0m") != null);
...
test "top-level help renders flags as compact aligned rows" {
    ...
    try std.testing.expect(std.mem.find(u8, wide, "Print the di version and exit\n\nExamples:") != null);
...
test "per-command help preserves long resume usage outside top-level index" {
    ...
    try std.testing.expect(std.mem.find(u8, text, "Usage:\n  di session resume [last|<id>] [--record] | session resume --id <id> [--record] | --resume [last|<id>] [--record] | resume [last|<id>] [--record] | resume --id <id> [--record] | --resume-last | --continue | -c | -r | --resume-<id>") != null);
    try std.testing.expect(std.mem.find(u8, text, "Options:") != null);
