...
pub const CommandHandlers = struct {
    ...
    revert_conversation: *const fn (ctx: *anyopaque) anyerror!void,
    unrevert_conversation: *const fn (ctx: *anyopaque) anyerror!void,
...
pub fn route(registry: SlashRegistry, handlers: *const CommandHandlers, cmd: []const u8) !void {
    switch (parse(registry, cmd)) {
        ...
        .revert => try handlers.revert_conversation(handlers.ctx),
        .unrevert => try handlers.unrevert_conversation(handlers.ctx),
...
fn testHandlers(ctx: *TestContext) CommandHandlers {
    return .{
        ...
        .revert_conversation = unexpectedNoPayload,
        .unrevert_conversation = unexpectedNoPayload,
