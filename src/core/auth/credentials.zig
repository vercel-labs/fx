...
pub const missing_credential_message = "di needs a model provider credential. Run di login to sign in, di setup to use an API key, or set OPENPATHS_API_KEY, OPENROUTER_API_KEY, or AI_GATEWAY_API_KEY.";
pub const missing_interactive_credential_message = "di needs a model provider credential. Run /login to sign in, /setup to use an API key, or set OPENPATHS_API_KEY, OPENROUTER_API_KEY, or AI_GATEWAY_API_KEY.";
...
pub fn resolvePreferring(
    ...
    const fx_login = loadFxLoginForPrecedence(alloc, transport, mode) catch |err| blk: {
        ...
        debug_trace.logf("auth", "di login load failed mode={t} err={s}; using precedence", .{ mode, @errorName(err) });
        break :blk null;
    };
...
test "a disabled store still reports why the di login was silent" {
    const alloc = std.testing.allocator;
    var fixture = try ExpiredFxLoginFixture.install(alloc);
    defer fixture.deinit();
    var store_fixture = SecretStoreFixture{ .disabled = true };
