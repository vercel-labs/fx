...
fn observeAuthFile(alloc: Allocator, fx_dir: *std.Io.Dir) !FileObservation {
    ...
    if (comptime builtin.os.tag == .windows) return .unusable;
    if (stat.kind != .file or stat.nlink != 1 or io_mod.permissionsToMode(stat.permissions) & 0o077 != 0) {
...
fn openExistingPrivateFxDir(home_dir: *io_mod.VerifiedDir) !io_mod.VerifiedDir {
    ...
    if (io_mod.permissionsToMode(initial_stat.permissions) & 0o200 == 0) {
