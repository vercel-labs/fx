...
/// Resume selector parsed from di ask --resume.
            ...
            if (!options.auto_next_steps and !options.auto_next_idea) {
                if (err == error.OneOffSessionNotResumable and !options.json_output) {
                    try deps.write_stderr(
                        deps.stderr_ctx,
                        "di ask: one-off child sessions cannot accept additional prompts; create a persistent child to continue the conversation\n",
                    );
                    return 1;
                }
    ...
    switch (attempt) {
        ...
        .failure => |failure| {
            ...
            try message.writer.print(
                "di ask: failed to save YOLO acknowledgment: {s}\n",
...
test "di ask ChatGPT route disables Gateway-backed auxiliary providers" {
    const alloc = std.testing.allocator;
    var stdout_capture = TestCapture{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture = TestCapture{};
...
test "di ask auto mode applies automatic allow and ask without a prompt" {
...
test "di ask ordinary multi-target admission preserves deny precedence without diagnostics" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
...
test "di ask renders one-off resume denial in text and JSON modes" {
    const alloc = std.testing.allocator;
    const cases = [_]struct {
...
test "di ask JSON permission-denied capture is best effort under allocation failure" {
    var failing = std.testing.FailingAllocator.init(
        std.testing.allocator,
        .{ .fail_index = 0 },
    );
...
test "di ask emits one discovery warning when catalog truncation and a skill read report it" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "di ask JSON preserves partial output on prompt failure" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
