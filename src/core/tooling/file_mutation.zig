...
fn validatePreimage(
    ...
    return switch (prepared.preimage) {
        ...
        .present => |expected| blk: {
            ...
            if (comptime builtin.os.tag != .windows and io_mod.permissionsToMode(stat.permissions) & 0o222 == 0) break :blk .io_failure;
            if (expected_permissions) |permissions| {
                if (comptime builtin.os.tag != .windows and io_mod.permissionsToMode(stat.permissions) != io_mod.permissionsToMode(permissions)) break :blk .stale;
            }
