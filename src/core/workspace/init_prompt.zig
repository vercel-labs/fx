...
const distilled_rules =
    ...
    \\Never reference ~/.fx paths, session state, or other machine-local details.
;

// Source: /Users/aditya/Workspace/skills/agents-md/SKILL.md (copied verbatim; @embedFile cannot reach paths outside src).
const full_skill_text =
    ...
    \\## Hard rules
    \\
    \\- **Never overwrite an existing AGENTS.md.** If one exists, read it, report what's there, and ask before changing anything.
    \\- **Keep it short.** Under ~300 lines absolute max; target 150–400 words. Every line goes into every session — unnecessary instructions raise reasoning cost 14–22% and dilute focus (frontier models reliably follow only ~150–200 instructions total).
    \\- **Ground everything in the repo.** No generic filler ("write clean code"). If a statement isn't verifiable from the repo, cut it.
    \\- **Ask about ambiguity, don't guess.** Two lockfiles? Unclear test runner? Ask the maintainer which is canonical.
    \\
    ...
    \\## Workflow
    \\
    \\1. **Check for existing AGENTS.md** (and CLAUDE.md/GEMINI.md). If present, stop and report — do not modify without approval.
...
fn build_prompt(alloc: std.mem.Allocator, body: []const u8, workspace_root: []const u8, extras: []const u8, mode: Mode) ![]u8 {
    ...
    if (extras.len > 0) {
        ...
        try out.appendSlice(alloc, extras);
        try out.appendSlice(alloc, "\n");
    }
    return try out.toOwnedSlice(alloc);
}

pub fn buildDistilled(alloc: std.mem.Allocator, workspace_root: []const u8, extras: []const u8, mode: Mode) ![]u8 {
    return try build_prompt(alloc, distilled_rules, workspace_root, extras, mode);
}

pub fn buildFull(alloc: std.mem.Allocator, workspace_root: []const u8, extras: []const u8, mode: Mode) ![]u8 {
    return try build_prompt(alloc, full_skill_text, workspace_root, extras, mode);
}

test "distilled create contains minimality, no-overwrite, workspace root and extras" {
    const alloc = std.testing.allocator;
    const prompt = try buildDistilled(alloc, "/repo/root", "focus on docs", .create);
...
test "distilled audit proposes diffs and differs from create" {
    const alloc = std.testing.allocator;
    const audit = try buildDistilled(alloc, "/repo/root", "", .audit);
    defer alloc.free(audit);
    const create = try buildDistilled(alloc, "/repo/root", "", .create);
...
test "full embeds skill text with mode and workspace root" {
    const alloc = std.testing.allocator;
    const create = try buildFull(alloc, "/repo/root", "", .create);
    defer alloc.free(create);
    const audit = try buildFull(alloc, "/repo/root", "extra notes", .audit);
    defer alloc.free(audit);
