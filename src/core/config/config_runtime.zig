...
pub const Settings = struct {
    ...
    pub fn deinit(self: *Settings, alloc: Allocator) void {
        ...
        if (self.opencode_model) |value| alloc.free(value);
        self.permission_rules.deinit(alloc);
        self.* = .{};
    }
};

pub fn savedModelForProvider(settings: *const Settings, provider: model_provider.ProviderId) ?[]const u8 {
...
pub fn userPatchForProviderSelection(
    provider: model_provider.ProviderId,
    model: []const u8,
) settings_store.UserSettingsPatch {
    return switch (provider) {
        .gateway => .{ .provider = provider, .model = model },
        .codex => .{ .provider = provider, .codex_model = model },
        .grok => .{ .provider = provider, .grok_model = model },
        .opencode => .{ .provider = provider, .opencode_model = model },
    };
}
...
pub const ConfigSources = struct {
    ...
    opencode_model: ConfigSource = .compiled_default,
...
fn updateConfigSources(sources: *ConfigSources, settings: Settings, source: ConfigSource) void {
    ...
    if (settings.opencode_model != null) sources.opencode_model = source;
...
fn parseProfileOnlyFields(
    ...
    if (root.object.get("opencode_model")) |model_value| {
        ...
        settings.opencode_model = try alloc.dupe(u8, model_value.string);
    }
...
fn mergeSettings(target: *Settings, incoming: *Settings, alloc: Allocator) void {
    ...
    if (incoming.opencode_model) |value| {
        if (target.opencode_model) |current| alloc.free(current);
        target.opencode_model = value;
        incoming.opencode_model = null;
    }
    if (incoming.permission_mode) |value| target.permission_mode = value;
