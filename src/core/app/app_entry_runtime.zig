...
fn runInteractiveWithDeps(comptime App: type, comptime cooperative: bool, alloc: Allocator, launch: *cli_surface.InteractiveLaunch, deps: RunDeps) !RunOutcome {
    ...
    var app = App.init(alloc, launch) catch |err| {
        switch (err) {
            ...
            error.SessionBusy => {
                writeStderr(deps, "di: another Fx process may be using this session (running or suspended); check other terminals or run jobs, then use fg or quit that process\n");
                return .{ .exit = 1 };
            },
            ...
            => {
                writeStderr(deps, "di: this session is being updated; wait a moment and retry\n");
                return .{ .exit = 1 };
            },
            error.OneOffSessionNotResumable => {
                writeStderr(deps, "di: one-off child sessions cannot accept additional prompts; create a persistent child to continue the conversation\n");
                return .{ .exit = 1 };
            },
...
test "app entry maps unavailable session state to one expected startup failure" {
    ...
    const cases = [_]struct {
        ...
        .{
            ...
            .message = "di: another Fx process may be using this session (running or suspended); check other terminals or run jobs, then use fg or quit that process\n",
        },
        .{
        ...
        .{
            ...
            .message = "di: this session is being updated; wait a moment and retry\n",
        },
        .{
        ...
        .{
            ...
            .message = "di: one-off child sessions cannot accept additional prompts; create a persistent child to continue the conversation\n",
        },
    };
