...
const BuildBudget = stream_provider.BuildBudget;
...
fn validateToolMessageHistory(alloc: Allocator, messages: []const ChatMessage) !void {
    for (messages) |message| {
        if (message.role != .tool) continue;
        if (message.tool_call_id == null or message.tool_call_id.?.len == 0) {
            _ = alloc;
            return error.InvalidToolMessageHistory;
        }
    }
}
