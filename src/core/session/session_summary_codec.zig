...
fn requirePrivateCacheDirectory(dir: std.Io.Dir) !void {
    ...
    if (stat.kind != .directory or
        io_mod.permissionsToMode(stat.permissions) & 0o777 != io_mod.permissionsToMode(private_dir_permissions))
    {
        return error.InvalidSessionIndex;
    }
}
...
fn readDeferredCacheTokenFromDirectory(
    ...
    if (stat.kind != .file or stat.nlink != 1 or
        (comptime builtin.os.tag != .windows and io_mod.permissionsToMode(stat.permissions) & 0o777 != io_mod.permissionsToMode(private_file_permissions)) or
...
pub fn refreshRelationshipMigrationSnapshot(
    ...
    if (target_stat.kind != .file or
        ...
        io_mod.permissionsToMode(target_stat.permissions) & 0o777 != 0o600)
    {
        return error.InvalidSessionIndex;
    }
...
fn openVerifiedSessionIndexFile(
    ...
    const stat = try file.stat(io_mod.getIo());
    if (stat.kind != .file or stat.nlink != 1 or (comptime builtin.os.tag != .windows and io_mod.permissionsToMode(stat.permissions) & 0o777 != 0o600)) {
        return error.InvalidSessionIndex;
    }
    return file;
}
