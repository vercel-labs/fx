...
fn openUsageRecoveryProfileRoot(
    ...
    if (stat.kind != .directory or
        io_mod.permissionsToMode(stat.permissions) & 0o777 != 0o700)
    {
...
fn validateUsageRecoveryMarker(
    ...
    if (stat.kind != .file or
        ...
        (comptime builtin.os.tag == .windows or io_mod.permissionsToMode(stat.permissions) & 0o777 != 0o600))
    {
        return error.InvalidUsageRecoveryIndex;
    }
...
test "same-workspace append defers latest cache contention and marks cache dirty" {
    ...
    try std.testing.expectEqual(@as(u32, 0o600), io_mod.permissionsToMode(token_stat.permissions) & 0o777);
...
test "missing home is empty for reads and bootstrapped privately for writes" {
    ...
    try std.testing.expectEqual(@as(u32, 0o700), io_mod.permissionsToMode(home_stat.permissions) & 0o777);
