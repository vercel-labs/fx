...
fn verifyPrivateDirectory(dir: std.Io.Dir) !void {
    ...
    if (comptime builtin.os.tag == .windows) return error.PrivateStatePermissionsUnsupported;
    if (io_mod.permissionsToMode(stat.permissions) & 0o777 != 0o700) {
        return error.PrivateStatePermissionsUnsupported;
    }
}
...
fn verifyPrivateOpenedStat(
    ...
    io_mod.verifyOpenedRegularFile(stat, mode) catch
        return error.SessionPathUnsafe;
    if (comptime builtin.os.tag == .windows) return error.PrivateStatePermissionsUnsupported;
    if (io_mod.permissionsToMode(stat.permissions) & 0o777 != 0o600) {
        return error.PrivateStatePermissionsUnsupported;
    }
}
...
test "terminal capabilities are private route restricted and reject symlinks" {
    ...
    try std.testing.expectEqual(@as(std.posix.mode_t, 0o700), io_mod.permissionsToMode(terminal_stat.permissions) & 0o777);
    try std.testing.expectEqual(@as(std.posix.mode_t, 0o700), io_mod.permissionsToMode(state_stat.permissions) & 0o777);
    try std.testing.expectEqual(@as(std.posix.mode_t, 0o600), io_mod.permissionsToMode(record_stat.permissions) & 0o777);
