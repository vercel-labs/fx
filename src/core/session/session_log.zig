...
pub const ConversationWriter = struct {
    ...
    fn initWithReplayScan(alloc: Allocator, file: std.Io.File, replay_scan: ?*ConversationReplayScan) !ConversationWriter {
        ...
        try writer.replayCommitted(replay_scan);
        return writer;
    }
    ...
    fn replayCommitted(writer: *ConversationWriter, replay_scan: ?*ConversationReplayScan) !void {
...
pub const LoadedWritableSession = struct {
    ...
    pub fn truncateConversation(
        ...
        try writer.replayCommitted(null);
        if (writer.turn_open) {
