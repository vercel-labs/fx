...
pub fn valid(text: []const u8, spans: []const Span) bool {
...
/// Caller owns the returned slice. Missing metadata denotes ordinary text.
pub fn parse(alloc: std.mem.Allocator, text: []const u8, value: ?std.json.Value) ![]Span {
    const raw = value orelse return &.{};
    var parsed = std.json.parseFromValue([]Span, alloc, raw, .{}) catch |err| switch (err) {
        error.OutOfMemory => return err,
        else => return error.InvalidSessionFormat,
    };
    defer parsed.deinit();
    if (!valid(text, parsed.value)) return error.InvalidSessionFormat;
    return alloc.dupe(Span, parsed.value);
}

pub fn write(writer: *std.Io.Writer, spans: []const Span) !void {
    if (spans.len == 0) return;
    try writer.writeAll(",\"paste_spans\":");
    try std.json.Stringify.value(spans, .{}, writer);
}

/// Caller owns the result. Invalid metadata falls back to complete text.
pub fn collapse(alloc: std.mem.Allocator, text: []const u8, spans: []const Span) ![]u8 {
...
pub fn collapsedOffset(text: []const u8, spans: []const Span, offset: usize) ?usize {
    var source: usize = 0;
    var target: usize = 0;
    for (spans) |span| {
...
test "paste display preserves surrounding text and literal lookalikes" {
    ...
    const spans = [_]Span{.{ .id = 7, .start = 7, .end = 11 }};
...
test "paste display previews complete lines and collapses only the remainder" {
    ...
    const spans = [_]Span{.{ .id = 1, .start = start, .end = text.len }};
