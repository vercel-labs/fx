...
const generation_origin = "https://opencode.ai/zen/go/v1";
const e2e_endpoint_env = "FX_E2E_OPENCODE_GO_RESPONSES_URL";
...
fn validateModel(model: []const u8) !void {
    if (model.len == 0 or model.len > 256) return error.InvalidOpenCodeGoModel;
    for (model) |byte| {
        if (byte <= 0x20 or byte == 0x7f) return error.InvalidOpenCodeGoModel;
    }
}
...
fn buildRequest(
    ...
    if (request.verified_images) |images| {
        if (images.len > 0) return error.OpenCodeGoVisionUnsupported;
    }
    ...
    try writeInput(writer, request.messages);
        ...
        switch (message.role) {
            ...
            .assistant => {
                ...
                for (message.tool_calls) |call| {
                    ...
                    {
                        return error.OpenCodeGoToolCallLimitExceeded;
                    }
                    try writeComma(writer, &first);
...
fn streamCompletionCore(alloc: Allocator, request: stream_provider.Request) !stream_provider.Result {
    ...
    if (request.credential_source != .opencode_api_key) {
        return error.OpenCodeGoCredentialRequired;
    }
    if (request.api_key.len == 0) return error.OpenCodeGoCredentialRequired;
    try validateModel(request.model);
    ...
    const request_endpoint = if (io_mod.getenv(e2e_endpoint_env)) |override| endpoint: {
        if (!gateway_client.isLoopbackHttpUrl(override)) return error.InvalidE2EOpenCodeGoEndpoint;
    ...
    if (response.head.status != .ok) {
        ...
        const bounded_body = reader.allocRemaining(alloc, .limited(max_error_body_bytes + 1)) catch |err| switch (err) {
            error.StreamTooLong => try alloc.dupe(u8, "OpenCode Go error response exceeded the local limit"),
            else => return err,
        };
        const body = if (bounded_body.len > max_error_body_bytes) body: {
            alloc.free(bounded_body);
            break :body try alloc.dupe(u8, "OpenCode Go error response exceeded the local limit");
        } else bounded_body;
        return .{
        ...
        while (true) {
            const fragment = reader.takeDelimiter('\n') catch |err| switch (err) {
                error.StreamTooLong => {
                    ...
                    if (buffered.len == 0) return error.OpenCodeGoSseReadStalled;
                    if (buffered.len > max_sse_line_bytes - self.pending_line.items.len) {
                        return error.OpenCodeGoSseEventTooLarge;
                    }
...
fn consumeSse(
    ...
    while (try sse.next(alloc, reader)) |json_text| {
        ...
        var parsed = std.json.parseFromSlice(std.json.Value, alloc, json_text, .{}) catch
            return error.InvalidOpenCodeGoSseEvent;
        defer parsed.deinit();
            ...
            std.mem.eql(u8, event_type, "response.refusal.delta"))
        {
            ...
            const output_index = integerField(parsed.value.object, "output_index") orelse -1;
            const delta = stringField(parsed.value.object, "delta") orelse continue;
            const index = findTool(tools.items, output_index) orelse
                (if (stringField(parsed.value.object, "call_id")) |call_id| findToolById(tools.items, call_id) else null) orelse continue;
            ...
            if (std.mem.eql(u8, item_type, "function_call")) {
                ...
                const name = stringField(item.object, "name") orelse continue;
                const output_index = integerField(parsed.value.object, "output_index") orelse -1;
                const index = findTool(tools.items, output_index) orelse findToolById(tools.items, call_id);
                const resolved_index = if (index) |found| found else blk: {
                    try appendTool(alloc, &tools, output_index, call_id, name);
                    if (on_tool_start) |callback| callback(callback_ctx, call_id, name, null);
                    break :blk tools.items.len - 1;
                };
                if (stringField(item.object, "arguments")) |arguments| {
                    const previous = tools.items[resolved_index].arguments.items;
                    if (std.mem.startsWith(u8, arguments, previous)) {
                        const suffix = arguments[previous.len..];
                        try appendToolArguments(alloc, &tools.items[resolved_index].arguments, suffix);
                        if (suffix.len > 0) if (on_tool_input_chunk) |callback| callback(callback_ctx, suffix);
                    } else if (!std.mem.eql(u8, arguments, previous)) {
                        tools.items[resolved_index].arguments.clearRetainingCapacity();
                        try appendToolArguments(alloc, &tools.items[resolved_index].arguments, arguments);
                    }
                }
        ...
        {
            return error.OpenCodeGoResponseFailed;
        }
    }
    if (cancel_flag.load(.seq_cst)) return error.Cancelled;
    if (!terminal_seen) return error.OpenCodeGoStreamIncomplete;
...
fn appendToolArguments(
    ...
    _ = checkedAccumulatedSize(arguments.items.len, delta.len, max_tool_arguments_bytes) catch
        return error.OpenCodeGoToolArgumentsTooLarge;
...
fn checkedAccumulatedSize(current: usize, additional: usize, maximum: usize) !usize {
    const next = std.math.add(usize, current, additional) catch
        return error.OpenCodeGoResourceLimitExceeded;
    if (next > maximum) return error.OpenCodeGoResourceLimitExceeded;
    return next;
}
...
fn findToolById(tools: []const ToolAccumulator, call_id: []const u8) ?usize {
    for (tools, 0..) |tool, index| if (std.mem.eql(u8, tool.id, call_id)) return index;
    return null;
}
fn finishReason(
    status: ?[]const u8,
...
test "OpenCode Go request uses Responses input and converts AI SDK tool schemas" {
    ...
    try std.testing.expect(std.mem.find(u8, body, "\"model\":\"opencode-model\"") != null);
    ...
    try std.testing.expect(std.mem.find(u8, body, "\"encrypted_content\"") == null);
...
test "OpenCode Go standard requests omit the priority service tier" {
...
test "OpenCode Go rejects verified images without vision capability" {
    ...
    try std.testing.expectError(
        error.OpenCodeGoVisionUnsupported,
...
test "OpenCode Go rejects wrong-origin and empty credentials before network I/O" {
    ...
    try std.testing.expectError(
        error.OpenCodeGoCredentialRequired,
...
test "OpenCode Go SSE maps text tools and usage without provider state" {
    const sse_text =
        ...
        "data: {\"type\":\"response.output_item.done\",\"output_index\":2,\"item\":{\"type\":\"function_call\",\"call_id\":\"call_1\",\"name\":\"read_file\",\"arguments\":\"{\\\"path\\\":\\\"README.md\\\"}\"}}\n\n" ++
...
test "OpenCode Go SSE recovers final message items without deltas" {
    const sse_text =
        "data: {\"type\":\"response.output_item.done\",\"output_index\":1,\"item\":{\"type\":\"message\",\"content\":[{\"type\":\"output_text\",\"text\":\"final\"}]}}\n\n" ++
    ...
    try std.testing.expectEqualStrings("final", capture.content.items);
    try std.testing.expectEqualStrings("final", completion.content.?);
}
...
test "OpenCode Go request deadline closes slow headers and stalled SSE" {
...
test "OpenCode Go error-body reader accepts the exact bound and replaces one beyond" {
    inline for (.{ TestResponseMode.error_body_exact, TestResponseMode.error_body_excess }) |mode| {
        ...
        if (mode == .error_body_exact) {
            ...
            try std.testing.expectEqualStrings(
                "OpenCode Go error response exceeded the local limit",
                result.err_body.?,
            );
        }
    }
...
test "OpenCode Go SSE reader accepts the exact line bound and rejects one beyond" {
    inline for (.{ max_sse_line_bytes, max_sse_line_bytes + 1 }) |line_bytes| {
        ...
        if (line_bytes == max_sse_line_bytes) {
            ...
            try std.testing.expectError(
                error.OpenCodeGoSseEventTooLarge,
...
test "OpenCode Go SSE reducer enforces event and ignored-wire aggregate bounds" {
    ...
    try expectTestSseError(error.OpenCodeGoResourceLimitExceeded, excess_events);
    ...
    try expectTestSseError(error.OpenCodeGoResourceLimitExceeded, excess_aggregate);
}

test "OpenCode Go SSE reducer enforces bounded tool arguments" {
    ...
    try expectTestSseError(error.OpenCodeGoToolArgumentsTooLarge, excess_arguments);
}
