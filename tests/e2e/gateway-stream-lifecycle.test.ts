...
describe("Qwen Gateway continuations", () => {
  const qwen = "alibaba/qwen3.8-max";

  function qwenEnv(root: FixtureRoot, gateway: GatewayFixture): Record<string, string | undefined> {
    return {
      ...Object.fromEntries(Object.keys(process.env).filter(key => key.startsWith("FX_")).map(key => [key, undefined])),
      HOME: root.home,
      AI_GATEWAY_API_KEY: "synthetic-qwen-test",
      VERCEL_OIDC_TOKEN: undefined,
      FX_DISABLE_KEYCHAIN: "1",
      FX_E2E_DISABLE_DOTENV: "1",
      FX_AUTO_UPGRADE: "0",
  ...
  function expectSuccess(result: Awaited<ReturnType<typeof runFx>>, expectedStderr = "") {
    expect(result.timedOut).toBe(false);
    expect(result.signal).toBeNull();
    expect(result.code).toBe(0);
  ...
  function expectInstructions(request: GatewayFixture["requests"][number]) {
    const prompt = gatewayRequest(request.body).prompt;
    ...
    if (request.headers.get("ai-language-model-id") === qwen) {
      expect(systems).toHaveLength(1);
      expect(contentText(systems[0]!.content)).toContain(WEB_SEARCH_GUIDANCE);
    } else {
      expect(systems.length).toBeGreaterThan(1);
      expect(systems.some(message => contentText(message.content) === WEB_SEARCH_GUIDANCE)).toBe(true);
    }
    return prompt;
  }

  test("preserves a tool-free saved-session continuation", async () => {
    ...
    const gateway = startDynamicFakeGateway(raw => {
      ...
      // The observed route accepted initial prompts but rejected multiple
      // systems once assistant history was present, even without tools.
      ...
      return fakeGatewayFinalText(calls++ === 0 ? firstReply : secondReply);
    }, { models: [{ id: qwen, type: "language", tags: ["tool-use"] }] });
    try {
      const env = qwenEnv(root, gateway);
      const first = expectSuccess(await runFx(["ask", "--json", "--auto", "--model", qwen, firstPrompt], {
        cwd: root.workspace, env, timeoutMs: 15_000,
      }));
      expect(first.output).toBe(firstReply);
      expect(first.tool_calls).toEqual([]);
      expect(first.session_id.length).toBeGreaterThan(0);
      expect(gateway.requests).toHaveLength(1);

      const second = expectSuccess(await runFx([
        "ask", "--json", "--auto", "--model", qwen, "--resume-id", first.session_id, secondPrompt,
      ], { cwd: root.workspace, env, timeoutMs: 15_000 }));
      expect(second.session_id).toBe(first.session_id);
      expect(second.output).toBe(secondReply);
      ...
      for (const [index, request] of gateway.requests.entries()) {
        ...
        const prompt = expectInstructions(request);
  ...
  test("single-system projection survives tools and model switches", async () => {
    ...
    try {
      const env = qwenEnv(root, gateway);
      const first = expectSuccess(await runFx(["ask", "--json", "--auto", "--model", qwen, "Read note.txt"], {
        cwd: root.workspace, env, timeoutMs: 15_000,
      }), "Reading note.txt\n");
      expect(first.session_id.length).toBeGreaterThan(0);
      expect(first.output).toBe("QWEN_NOTE_OK");
      expect(first.tool_calls).toEqual([{ name: "read_file", status: "success" }]);
      expect(gateway.requests).toHaveLength(2);
      expect(toolResultOutput(gateway.requests[1]!.body, "qwen_read")).toContain("QWEN_NOTE_OK");
      for (const request of gateway.requests) {
        expect(request.headers.get("ai-language-model-id")).toBe(qwen);
        expectInstructions(request);
      }

      for (const [index, model] of [MODEL, qwen].entries()) {
        expectQwen = model === qwen;
        const result = expectSuccess(await runFx([
          "ask", "--json", "--auto", "--model", model, "--resume-id", first.session_id, `Follow-up ${index}`,
        ], { cwd: root.workspace, env, timeoutMs: 15_000 }));
        expect(result.session_id).toBe(first.session_id);
        expect(result.output).toBe("QWEN_NOTE_OK");
