...
async function launchAutomaticRecording(): Promise<{
  ...
  const tracePath = join(workDir, "trace.log");
  const s = await TmuxSession.create({
    cmd: `env -u AI_GATEWAY_API_KEY -u VERCEL_OIDC_TOKEN -u OPENAI_API_KEY -u LITELLM_API_KEY FX_DISABLE_KEYCHAIN=1 FX_SKIP_ONBOARDING=1 ${FX_BIN} --record`,
    cwd: workDir,
    width: 180,
    height: 36,
    env: {
...
describe("tui: render record/replay", () => {
  ...
  test.skipIf(SKIP)(
    ...
    async () => {
      ...
      const authNotice =
        "● Auth: Fx needs model access. Run /login to sign in, /setup to use a Gateway API key, set AI_GATEWAY_API_KEY, or set OPENAI_API_KEY for an OpenAI-compatible server.";
      ...
      await session.waitForText("● Auth: Fx needs model access", 5_000);
