...
const allowlist: AllowRule[] = [
  ...
  rule("src/ui/remote_attach.zig", "(?:enterTui|leaveTui|erasePreviousMutableUnlocked|renderTuiUnlocked)", /stdio_(?:acquisition|acquisition_write|write)/, "interactive_terminal_owner", "inline semantic remote conversation lifecycle and normal-style frame delivery"),
  rule("src/ui/remote_attach.zig", "(?:renderSnapshot|renderMessage|renderUpdate|renderToolUnlocked|renderPendingUnlocked|renderRunState|renderOperation|renderError|sendPrompt|reportInputError|inputMainLines)", /stdio_(?:acquisition|acquisition_write|write)/, "noninteractive_output", "redirected semantic remote presentation and diagnostics"),
  rule("src/ui/remote_attach.zig", "(?:refreshTuiGeometryUnlocked|runWithWire)", /fixed_descriptor/, "terminal_probe", "remote attach TTY and geometry capability probe"),
