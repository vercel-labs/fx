...
const allowlist: AllowRule[] = [
  ...
  rule("src/core/execution/command_runner.zig", "runForegroundSessionBootstrap", /stdio_acquisition_write/, "subprocess_protocol_transport", "foreground command bootstrap protocol"),
  rule("src/core/execution/command_runner.zig", "(?:superviseSessionHelpers|exitForegroundSessionFailure|writeForegroundSessionReplaceFailure)", /^stdio_acquisition$/, "subprocess_protocol_transport", "private supervisor failure transport and captured-stream closure"),
  rule("src/core/execution/managed_execution.zig", "deinitWithOutcome", /^stdio_acquisition_write$/, "initialization_teardown", "failed session-helper retirement diagnostic"),
