...
After CI passes for a push to `main`, the dev release workflow publishes commit-addressed binaries and then updates `dev.json`. Dogfooders opt in with `fx upgrade --channel dev`; the choice is stored in their user settings and applies to manual upgrades, automatic upgrades, and the `ctrl+g` handoff. `fx upgrade --channel stable` returns to tagged releases. Dev publishing does not create tags or GitHub Releases. Dev auto-upgrade continues polling at one-minute intervals while an installed update awaits `ctrl+g`, replacing superseded builds without restarting the conversation. Unchanged or unavailable manifests preserve the ready prompt; a failed replacement preserves the installed executable and retries later without advertising the superseded build as ready.
